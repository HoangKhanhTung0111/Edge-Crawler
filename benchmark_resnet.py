...
stacked = np.stack([single] * BATCH, axis=0)  # (N,C,H,W)
batch_input = np.ascontiguousarray(np.transpose(stacked, (1, 0, 2, 3)))  # (C,N,H,W) for ncnn Mat
...
def bench(use_vulkan, label):
    ...
    net.load_param('resnet50_b8.ncnn.param')
    net.load_model('resnet50_b8.ncnn.bin')
    ...
    print(f"{label} top1 (anh 0): idx={idx} label={labels[idx] if idx < len(labels) else '?'} score={out_arr[0][idx]:.2f}")
    return per_image
